...
The DART documentation now lives on the unified documentation site at
https://docs.aptrust.org/dart-docs/, which is built by the APTrust/aptrust-docs
repo from the markdown in this repo's docs/ directory.

The old standalone site at https://aptrust.github.io/dart-docs/ no longer serves
the documentation. Instead it serves one redirect stub per page, plus a catch-all
...
as well as for pages:
    ...
    https://aptrust.github.io/dart-docs/<path>
      -> https://docs.aptrust.org/dart-docs/<path>
...
# Path GitHub Pages serves this repo under, e.g. aptrust.github.io/dart-docs/.
# Used by 404.html to strip the old base off the incoming path.
OLD_PREFIX = "/dart-docs/"

# Where the documentation lives now. Must end with a slash. The path segment
# happens to match OLD_PREFIX here, but the two are independent: OLD_PREFIX is
# about aptrust.github.io, NEW_BASE is about docs.aptrust.org.
NEW_BASE = "https://docs.aptrust.org/dart-docs/"
...
<title>Page moved &mdash; APTrust DART Docs</title>
...
<p>The DART documentation now lives on the
...
<p>The DART documentation now lives on APTrust's
...
def url_path_for(md_file):
    ...
    non-empty, ends with a slash:
        ...
        docs/index.md                     -> ""
        docs/runner/index.md              -> "runner/"
        docs/dart2/videos.md              -> "dart2/videos/"
        docs/dart3/users/jobs/upload.md   -> "dart3/users/jobs/upload/"
    """
...
def main():
    ...
    # also skips nav entries that point at external URLs, which have no page
    # here and need no stub.
